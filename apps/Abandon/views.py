...
class SecretaryView(LoginRequiredMixin, View):
	template_name = "abandon_secr_edit.html"
...
class DocumentFormView(LoginRequiredMixin, View):
	...
	def post(self, request, *args, **kwargs):
		...
		if "submit" in request.POST:
			if form.is_valid():
				abandon = form.save(commit=False)
				abandon.user = request.user
				abandon.save()
				messages.success(request, "Document cree avec Succes ! ")
				return redirect(BASE_NAME+"_payform", abandon=abandon.id)
			return render(request, self.template_name, locals())
		if form.is_valid():
			abandon = form.save(commit=False)
			abandon.user = request.user
...
class DocumentPayView(LoginRequiredMixin, View):
	template_name = "abandon_pay_form.html"

	def get(self, request, abandon, *args, **kwargs):
		payform = BASE_NAME+"_payform"
		document = Document.objects.get(id=abandon)
		if document.zone_payment:
			return redirect(BASE_NAME+"_list")
		form = PaymentZoneForm()
		return render(request, self.template_name, locals())

	def post(self, request, abandon, *args, **kwargs):
		payform = BASE_NAME+"_payform"
		document = Document.objects.get(id=abandon)
...
def update_document(request, id): 
    ...
    if request.user == document.user:
        if form1.is_valid():
            ...
            form.save() 
            print(Document.payment_percent)
...
def delete_document(request, document_id, template_name='abandon_delete_form.html'):
