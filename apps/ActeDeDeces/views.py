...
class DocumentPayView(LoginRequiredMixin, View):
	template_name = "deces_pay_form.html"

	def get(self, request, deces, *args, **kwargs):
		payform = BASE_NAME+"_payform"
		document = Document.objects.get(id=deces)
		if document.zone_payment:
			return redirect(BASE_NAME+"_list")
		form = PaymentZoneForm()
		return render(request, self.template_name, locals())

	def post(self, request, deces, *args, **kwargs):
		payform = BASE_NAME+"_payform"
		document = Document.objects.get(id=deces)
...
def delete_document(request, document_id, template_name='deces_delete_form.html'):
...
def update_document(request, id): 
    ...
    context["form"] = form1 
    return render(request, "deces_update_form.html", context) 
